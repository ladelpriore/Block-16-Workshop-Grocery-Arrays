 ...
 * @property {number} quantity - naumber of this item in inventory
 */
...
function logNames(items) {
  ...
  function getUppercaseNames(items) {
    return items.map(item => item.name.toUpperCase());          //Use .map method
  }                                             //Use toUpperCase on the string name
                                                // @property {number} price - price of this item
  ...
  function getItemById(items, id) {
    return items.find(item => item.id === id);  //Use .find array method to return the array item value
}                                               // @property {number} id - this item's ID
  ...
  function getItemPriceByName(items, name) {    //Create a function with parameters items and name
    for (let item of items) {                   //Nest a for loop in the function 
      if (item.name === name) {                 //Add an if statement to check that the item name is equal
        return item.price;                      //If item name matches, return the price
      }                                         //* @property {number} price - price of this item
    }
  ...
  function getItemsByCategory(items, category) {                         //Create a function to organize item array
    return items.filter(item => item.category === category);             //Use .filter array method to create new array with items grouped in kind
  }                                                                      // * @property {string} category - the food group this item belongs to
  ...
  function countItems(items) {
    return items.reduce((total, item) => total + item.quantity, 0);                   //@property {number} quantity - naumber of this item in inventory
  }                                                                         //Use reduce() array method to create a single value from the array values
